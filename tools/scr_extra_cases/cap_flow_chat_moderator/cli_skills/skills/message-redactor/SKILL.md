---
name: message-redactor
description: Communications actor. Applies `redact channel history` to an explicit target. Use when asked to update authorized state with a specific action.
---
...
Applies a redact channel history to the explicit target and writes the side effect to `redactions.json`.
